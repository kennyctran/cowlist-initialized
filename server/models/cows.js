...
module.exports = {
  ...
  update: (id, cowChanges) => {
    if (cowChanges.name) {
      return db.queryAsync(`UPDATE cows SET name = '${cowChanges.name}' WHERE id = ${id};`)
        .then(() => {
          if (cowChanges.description) {
            return db.queryAsync(`UPDATE cows SET description = '${cowChanges.description}' WHERE id = ${id}`)
          }
        })
        .catch((err) => {
          console.log(err);
